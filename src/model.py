# Authors: Emily Proctor and David Noblett
# Purpose: This script implements a bidirectional LSTM model for per-residue protein secondary structure prediction given primary amino acid sequence.
...
parser.add_argument('--mode', type=str, choices=['train', 'inference'], default='train', help="Choose 'train' to train a new model or 'inference' to run predictions with a saved model.")
parser.add_argument('--fasta', type=str, default=None, help='Path to a FASTA file containing protein sequences for inference.')
...
parser.add_argument('--num_outputs', type=int, default=9, help='Number of output classes for DSSP  (3 or 9).')
parser.add_argument('--seed', type=int, default=42, help='Random seed.')
parser.add_argument('--output_folder', type=str, help='output folder where all files will be saved.')
...
def read_dataset_file():
    with open('dataset_25000.json', 'r') as file: 
...
def train_model(batch_size=32, hidden_dim=64, num_layers=2, learning_rate=0.001, num_epochs=150, dropout=0.2, end_training_threshold=5, num_outputs=9):
    ...
    log_file = open(f"{output_folder}/training_log", "w")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_sequences, batch_labels, mask in train_dataloader:
            ...
            loss = loss_func(outputs, batch_labels) # average loss over all non-padded positions in the batch
            loss.backward() # compute gradients
            optimizer.step() # update weights

            num_valid_tokens = batch_labels.size(0)
            total_loss += loss.item() * num_valid_tokens  # sum loss weighted by number of valid tokens
            total_tokens += num_valid_tokens # count valid tokens for averaging later
        ...
        with torch.no_grad():  # no gradient calculation for validation
            for val_sequences, val_labels, val_mask in val_dataloader:
                ...
                val_outputs = model(val_sequences) # forward pass
                val_outputs = val_outputs.view(-1, num_dssp)
                val_labels = val_labels.view(-1)
                val_mask  = val_mask.view(-1)

                val_outputs = val_outputs[val_mask] # apply mask to filter out padded positions
                val_labels  = val_labels[val_mask]

                loss = loss_func(val_outputs, val_labels)

                num_valid_tokens = val_labels.size(0)
                val_total_loss += loss.item() * num_valid_tokens
                val_total_tokens += num_valid_tokens

                _, val_predicted = torch.max(val_outputs, dim=1) # get predicted classes
                val_all_preds.extend(val_predicted.cpu().numpy()) # store validation predictions
                val_all_labels.extend(val_labels.cpu().numpy()) # store validation labels
        ...
        log_file.write(
            (
                f"{epoch+1}\t{end_time - start_time:.2f}\t{avg_loss:.4f}\t"
                f"{train_metrics['balanced_accuracy']:.4f}\t{train_metrics['precision']:.4f}\t"
                f"{train_metrics['recall']:.4f}\t{train_metrics['f1']:.4f}\t{val_loss:.4f}\t"
                f"{val_metrics['balanced_accuracy']:.4f}\t{val_metrics['precision']:.4f}\t"
        ...
        if val_bal_acc > best_val_acc:
            ...
            if epochs_without_improvement >= end_training_threshold:
                print(f"early stopping triggered at epoch {epoch+1}, best val balacc: {best_val_acc:.4f}")
                break
    ...
    return model, test_dataloader, val_metrics
...
if __name__ == "__main__":
    ...
    aa_options = "ACDEFGHIKLMNPQRSTVWY" # 20 canonical amino acids
    index_to_aa = {aa: index + 1 for index, aa in enumerate(aa_options)} # start indexing from 1
    index_to_aa['PAD'] = 0 # padding token
    num_aa = len(aa_options) + 1 # num amino acids + padding
    ...
    if args.mode == 'train':
        ...
        if os.path.exists(f"{output_folder}/lstm_weights.pt"):
            overwrite = input(f"model already exists at {output_folder}/lstm_weights.pt. would you like to overwrite and retrain? (y/n): ").lower()
            if overwrite != 'y':
                print("training cancelled and keeping existing model")
                exit(0)

        # Train model
        model, test_dataloader, best_val_metrics = train_model(
        ...
        torch.save(model.state_dict(), f"{output_folder}/lstm_weights.pt")
        print(f"\best model saved to {output_folder}/lstm_weights.pt")

        # Evaluate on test set
        loss_func = nn.CrossEntropyLoss()
        test_metrics = evaluate_model(model, test_dataloader, loss_func, device, num_dssp)
        test_results_file = open(f"{output_folder}/test_results.txt", "w")
        test_results_file.write(f"balanced accuracy: {test_metrics['balanced_accuracy']:.4f}\n")
        test_results_file.write(f"precision: {test_metrics['precision']:.4f}\n")
        test_results_file.write(f"recall: {test_metrics['recall']:.4f}\n")
        test_results_file.write(f"F1: {test_metrics['f1']:.4f}\n")
    ...
    ##### INFERENCE MODE #####
        ...
        if not os.path.exists(f"{output_folder}/lstm_weights.pt"):
            print(f"no saved model found at {output_folder}/lstm_weights.pt - please train a model first")
            exit(1)

        if args.fasta is None:
            print("please provide FASTA file with --fasta <path_to_file>")
            exit(1)

        # Load trained model
        model = LSTMModel(input_dim=num_aa, hidden_dim=args.hidden_dim,
                        output_dim=num_outputs, num_layers=args.num_layers,
                        dropout=args.dropout)
        model.load_state_dict(torch.load(f"{output_folder}/lstm_weights.pt", map_location=device))
        model.to(device)
        model.eval()
        print(f"loaded model from {output_folder}/lstm_weights.pt")
        ...
        print(f"loaded {len(sequences)} protein sequences from {args.fasta}")

        index_to_dssp = {v: k for k, v in dssp_to_index.items()}

        convert_3class = {
            "H": "H", "G": "H", "I": "H",     # helix
            "E": "B", "B": "B",              # beta strand
            "T": "C", "S": "C", "P": "C", "0": "C"   # coil / unknown
        }

        # convert numeric class predictions to correct SS letters
        def decode_prediction(pred_array):
            raw = [index_to_dssp[i] for i in pred_array]

            if args.num_outputs == 3:
                return ''.join(convert_3class[s] for s in raw)
            else:
                return ''.join(raw)

        with open(f"{output_folder}/lstm_predictions.fasta", "w") as prediction_file:

            for seq_id, seq in sequences.items():
                try:
                    seq_indices = [index_to_aa.get(aa, 0) for aa in seq]
                    seq_tensor = torch.tensor([seq_indices], dtype=torch.long).to(device)

                    with torch.no_grad():
                        preds = torch.argmax(model(seq_tensor), dim=-1).cpu().numpy()[0]

                    decoded = decode_prediction(preds)

                    prediction_file.write(f">{seq_id}\n{decoded}\n")

                except Exception as e:
                    print(f"skipping {seq_id} due to error: {e}")

        print(f"\ninference complete and results saved to {output_folder}/lstm_predictions.fasta")